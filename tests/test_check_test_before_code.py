...
# ---------------------------------------------------------------------------
class TestWiring:
    def test_registered_in_pretooluse_edit_write_matcher(self):
        cfg = json.loads(
        ...
        for grp in pretool:
            ...
            for c in cmds:
                if "check_test_before_code" in c or "check-test-before-code" in c:
                    return
        raise AssertionError(
            "check_test_before_code must be registered on the "
            "Edit|Write|MultiEdit PreToolUse matcher in .claude/settings.json"
        )

    def test_routed_through_hooks_runner(self):
        """PreToolUse gate hooks go through hooks_runner.py (not direct wire).

        The runner normalizes dash/underscore filenames, applies HOOK_TIMEOUT,
        and emits hook events to the live dashboard. Direct wiring would
        bypass all of this."""
        cfg = json.loads(
            (REPO_ROOT / ".claude" / "settings.json").read_text(encoding="utf-8"))
        pretool = cfg["hooks"].get("PreToolUse", [])
            ...
            for h in grp.get("hooks", []):
                ...
                if "check_test_before_code" in c or "check-test-before-code" in c:
                    assert "hooks_runner" in c, (
                        "PreToolUse gate hooks must go through hooks_runner.py"
                    )
                    return
        # If not found, fail (the previous test would have caught it)
        raise AssertionError("check_test_before_code not found in PreToolUse")

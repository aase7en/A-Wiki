# Plan — GAS Personnel Check-in & Grouping Web App (รพ.อุทัย)

## Context
Build a container-bound Google Apps Script web app for personnel check-in and grouping at Uthai Hospital (รพ.อุทัย). Optimized for tablet/kiosk use at live events. The live master Google Sheet replaces `รายชื่อเจ้าหน้าที่ รพ.อุทัย.xlsx`. Personnel names live in the user's Sheet only — the repo holds public-safe code (no PII).

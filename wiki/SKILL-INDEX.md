...
| `agentic-engineering` | none | ecosystem | ทำงานเป็นวิศวกร agentic โดยใช้การประเมินผลก่อน execute, การแยกย่อยงาน, และการเลือกโมเดลที่คำนึงถึ... |
| `agentic-os` | none | ecosystem | สร้างระบบปฏิบัติการแบบ multi-agent ที่ทำงานต่อเนื่องบน Claude Code ครอบคลุมสถาปัตยกรรมเคอร์เนล, s... |
| `ai-first-engineering` | none | ecosystem | รูปแบบการทำงานของทีมวิศวกรรมที่เน้นให้ AI agent เป็นผู้สร้างโค้ดส่วนใหญ่ โดยทีมจะเน้นที่การออกแบบ... |
| `ai-regression-testing` | none | ecosystem | กลยุทธ์การทดสอบ regression สำหรับการพัฒนาโดยใช้ AI รองรับการทดสอบ API ในโหมด sandbox ที่ไม่ต้องพึ... |
...
| `backend-patterns` | none | ecosystem | design pattern ฝั่ง backend — API design, database optimization, server-side best practice สำหรับ... |
| `blender-motion-state-inspection` | none | ecosystem | ใช้ตรวจสอบสถานะการเคลื่อนไหวของตัวละครใน Blender เช่น ท่าโพส การ retarget แอนิเมชัน การสัมผัสพื้น... |
...
| `bun-runtime` | none | ecosystem | รันไทม์ Bun ที่รวมทั้ง package manager, bundler และ test runner ไว้ในตัวเดียว ใช้แทน Node.js ได้ท... |
| `ci-cd-and-automation` | ship | engineering-lifecycle | ตั้ง CI/CD pipeline — auto test, lint, build, deploy เพื่อลดมนุษย์ผิดพลาด |
| `cisco-ios-patterns` | none | ecosystem | รวบรวมรูปแบบคำสั่ง show, โครงสร้าง config, wildcard mask, การวาง ACL, การดูแล interface และการตรว... |
| `ck` | none | ecosystem | บันทึกและโหลดบริบทของโปรเจกต์โดยอัตโนมัติเมื่อเริ่ม session ใหม่ ทำงานร่วมกับ git เพื่อติดตามกิจก... |
...
| `coding-standards` | none | ecosystem | กำหนดมาตรฐานการเขียนโค้ดข้ามโปรเจกต์ ครอบคลุมการตั้งชื่อ ความอ่านง่าย การใช้ immutable และการตรวจ... |
| `compose-multiplatform-patterns` | none | ecosystem | รวบรวมรูปแบบการพัฒนา UI ด้วย Compose Multiplatform และ Jetpack Compose สำหรับโปรเจกต์ KMP ครอบคลุ... |
| `configure-ecc` | none | ecosystem | ตัวช่วยติดตั้ง Everything Claude Code แบบโต้ตอบ — นำทางผู้ใช้ในการเลือกและติดตั้ง skills และ rule... |
| `connections-optimizer` | none | ecosystem | วิเคราะห์และเสนอ [[wikilinks]] ระหว่างหน้า เพื่อให้ knowledge graph แน่นขึ้น |
...
| `content-hash-cache-pattern` | none | ecosystem | แคชผลลัพธ์การประมวลผลไฟล์ที่ใช้ทรัพยากรมากด้วย SHA-256 hash ของเนื้อหา โดยไม่ขึ้นกับ path และ inv... |
...
| `dashboard-builder` | none | ecosystem | สร้าง monitoring dashboards ที่ตอบคำถามจริงของ operator สำหรับ Grafana, SigNoz และแพลตฟอร์มอื่น ๆ... |
...
| `deployment-patterns` | none | ecosystem | รวบรวมรูปแบบการ deploy, CI/CD pipeline patterns, Docker containerization, health checks, rollback... |
...
| `django-celery` | none | ecosystem | ช่วยออกแบบและกำหนดค่า Celery ใน Django สำหรับงาน async เช่น task design, beat scheduling, retries... |
...
| `django-security` | none | ecosystem | security ของ Django — auth, authorization, CSRF, SQL injection, ป้องกันช่องโหว่ทั่วไป |
| `django-tdd` | none | ecosystem | กลยุทธ์ test Django ด้วย pytest-django + factory_boy + mocking + coverage — TDD แบบ Django |
...
| `dmux-workflows` | none | ecosystem | จัดการ orchestration ของ AI agent หลายตัวพร้อมกันด้วย dmux ซึ่งเป็น tmux pane manager ที่ออกแบบมา... |
| `docker-patterns` | none | ecosystem | รวบรวมรูปแบบการใช้งาน Docker และ Docker Compose สำหรับการพัฒนาในเครื่อง การรักษาความปลอดภัยของ co... |
...
| `documentation-lookup` | none | ecosystem | ดึง docs ล่าสุดของ lib/framework ผ่าน context7 MCP แทนการใช้ข้อมูลในโมเดล — กันใช้ API เก่า |
| `dotnet-patterns` | none | ecosystem | ให้คำแนะนำเกี่ยวกับรูปแบบการเขียนโค้ด C# และ .NET ที่เป็นมาตรฐาน ครอบคลุม dependency injection, a... |
...
| `ecc-guide` | none | ecosystem | แนะนำผู้ใช้เกี่ยวกับ agents, skills, commands, hooks, rules, install profiles และการเริ่มต้นโปรเจ... |
| `ecc-tools-cost-audit` | none | ecosystem | ตรวจสอบการใช้งาน ECC Tools อย่างละเอียด โดยเน้นหลักฐานจาก logs และ billing เพื่อหาสาเหตุของ PR ที... |
| `enterprise-agent-ops` | none | ecosystem | จัดการ workload ของ AI Agent ที่ทำงานระยะยาวในระดับองค์กร พร้อมการตรวจสอบ observability, การกำหนด... |
| `error-handling` | none | ecosystem | รูปแบบการจัดการข้อผิดพลาดที่แข็งแกร่งสำหรับ TypeScript, Python และ Go ครอบคลุม typed errors, erro... |
...
| `exa-search` | none | ecosystem | ใช้ Exa MCP เพื่อค้นหาเว็บ โค้ด และข้อมูลบริษัทด้วย neural search เหมาะสำหรับการค้นหาข้อมูลเชิงลึ... |
| `fastapi-patterns` | none | ecosystem | รวบรวมแพทเทิร์นสำหรับสร้าง FastAPI async APIs ทั้ง dependency injection, Pydantic models, OpenAPI... |
| `flox-environments` | none | ecosystem | สร้างสภาพแวดล้อมการพัฒนาที่ทำซ้ำได้และข้ามแพลตฟอร์มด้วย Flox ซึ่งเป็นตัวจัดการสภาพแวดล้อมแบบประกา... |
| `flutter-dart-code-review` | none | ecosystem | รายการตรวจสอบการ review โค้ด Flutter/Dart ที่ไม่ยึดติดกับ library ใด library หนึ่ง ครอบคลุมแนวทาง... |
...
| `github-ops` | none | ecosystem | จัดการและดำเนินการกับ GitHub repository โดยอัตโนมัติ ครอบคลุมการจัดการ Issue, PR, CI/CD, การปล่อย... |
| `golang-patterns` | none | ecosystem | แนวทางเขียนโค้ดภาษา Go ที่ใช้กันทั่วไป — concurrency ด้วย goroutine และ channel, การจัดการ error,... |
...
| `google-workspace-ops` | none | ecosystem | ดำเนินการกับ Google Drive, Docs, Sheets และ Slides เป็นพื้นผิวการทำงานเดียวสำหรับแผนงาน ตัวติดตาม... |
| `healthcare-cdss-patterns` | none | ecosystem | รูปแบบการพัฒนา Clinical Decision Support System (CDSS) สำหรับตรวจสอบปฏิกิริยาระหว่างยา ตรวจสอบขนา... |
...
| `hermes-imports` | none | ecosystem | แปลงเวิร์กโฟลว์ Hermes ในเครื่องให้เป็นสกิล ECC ที่ปลอดภัยและสร้าง artifacts สำหรับ release-pack ... |
| `hook-suggest` | none | wiki | แนะนำ hook ที่เหมาะสมสำหรับการทำงานต่าง ๆ ในโปรเจกต์ เช่น pre-commit, pre-push หรือ post-checkout... |
| `hookify-rules` | none | ecosystem | ทักษะนี้ใช้สำหรับสร้างหรือกำหนดค่า hookify rule ซึ่งเป็นกฎที่ใช้ในการจัดการ hook ภายในโปรเจกต์ ช่... |
...
| `ios-icon-gen` | none | ecosystem | สร้าง iOS app icons เป็น PNG imagesets สำหรับ Xcode asset catalogs โดยใช้ SF Symbols (มากกว่า 5,0... |
...
| `java-coding-standards` | none | ecosystem | รวบรวมแนวทางการเขียนโค้ด Java สำหรับ Spring Boot และ Quarkus ครอบคลุมการตั้งชื่อ, immutability, ก... |
| `jira-integration` | none | ecosystem | ใช้สำหรับดึงข้อมูล Jira tickets วิเคราะห์ requirements อัปเดตสถานะ เพิ่ม comment หรือ transition ... |
| `jpa-patterns` | none | ecosystem | รวบรวมแนวทางการออกแบบ Entity JPA/Hibernate สำหรับ Spring Boot ครอบคลุมการกำหนดความสัมพันธ์ การเพิ... |
| `knowledge-ops` | none | ecosystem | จัดการฐานความรู้แบบหลายชั้น ทั้งไฟล์ในเครื่อง, MCP memory, vector store และ Git repo รองรับการเพิ... |
| `kotlin-coroutines-flows` | none | ecosystem | coroutine + Flow ของ Kotlin สำหรับ Android/KMP — structured concurrency, Flow operator, state/col... |
| `kotlin-exposed-patterns` | none | ecosystem | ทักษะนี้รวบรวมแพทเทิร์นการใช้งาน JetBrains Exposed ORM สำหรับ Kotlin ครอบคลุมการเขียน DSL queries... |
| `kotlin-ktor-patterns` | none | ecosystem | รวบรวมรูปแบบการพัฒนา Ktor server ครอบคลุมการใช้งาน routing DSL, plugins, authentication, Koin DI,... |
| `kotlin-patterns` | none | ecosystem | แนวทางเขียนโค้ดภาษา Kotlin ที่นิยม — coroutine, scope function, data class, sealed class, null sa... |
...
| `laravel-plugin-discovery` | none | ecosystem | ค้นหาและประเมินแพ็กเกจ Laravel ผ่าน LaraPlugins.io MCP ใช้เมื่อต้องการค้นหา plugin ตรวจสอบสุขภาพแ... |
| `laravel-security` | none | ecosystem | รวมแนวทางปฏิบัติเพื่อความปลอดภัยใน Laravel ครอบคลุมการยืนยันตัวตน การอนุญาต การตรวจสอบข้อมูล CSRF... |
| `laravel-tdd` | none | ecosystem | ทักษะสำหรับทำ Test-Driven Development ใน Laravel โดยใช้ PHPUnit หรือ Pest ครอบคลุมการสร้าง factor... |
| `mcp-server-patterns` | none | ecosystem | สร้าง MCP server ด้วย Node/TypeScript SDK — tools, resources, prompts, Zod validation, stdio vs s... |
| `messages-ops` | none | ecosystem | สกิลนี้ช่วยให้คุณอ่านข้อความหรือ DM ที่ได้รับ ลิงก์รหัส OTP ที่เพิ่งส่งมา และตรวจสอบเธรดก่อนตอบกล... |
...
| `nanoclaw-repl` | none | ecosystem | ใช้ควบคุมและขยาย NanoClaw v2 ซึ่งเป็น REPL แบบ zero-dependency ที่ทำงานร่วมกับ claude -p สำหรับจั... |
| `nestjs-patterns` | none | ecosystem | รวบรวมรูปแบบสถาปัตยกรรม NestJS สำหรับการสร้างโมดูล คอนโทรลเลอร์ โพรไวเดอร์ การตรวจสอบ DTO การ์ด อ... |
...
| `nuxt4-patterns` | none | ecosystem | รูปแบบการพัฒนาแอป Nuxt 4 ที่เน้นความปลอดภัยของ hydration, ประสิทธิภาพ, route rules, lazy loading ... |
...
| `product-capability` | none | ecosystem | แปลงความตั้งใจจาก PRD, คำขอใน roadmap หรือการพูดคุยเกี่ยวกับผลิตภัณฑ์ให้เป็นแผนความสามารถที่พร้อม... |
| `product-lens` | none | ecosystem | ใช้สกิลนี้เพื่อตรวจสอบ 'เหตุผล' ก่อนเริ่มพัฒนา วินิจฉัยปัญหาของ product และทดสอบทิศทางของ product... |
...
| `python-patterns` | none | ecosystem | รวบรวม idioms, PEP 8, type hints และแนวปฏิบัติที่ดีที่สุดของ Python เพื่อสร้างโค้ดที่แข็งแกร่ง มี... |
...
| `pytorch-patterns` | none | ecosystem | รวบรวมแพทเทิร์นและแนวปฏิบัติที่ดีที่สุดสำหรับ PyTorch ในการสร้าง training pipeline, model archite... |
| `quarkus-patterns` | none | ecosystem | รูปแบบสถาปัตยกรรม Quarkus 3.x LTS สำหรับพัฒนา Java backend โดยใช้ Camel สำหรับ messaging, RESTful... |
| `quarkus-security` | none | ecosystem | รวบรวมแนวทางปฏิบัติที่ดีที่สุดด้านความปลอดภัยสำหรับ Quarkus ครอบคลุมการยืนยันตัวตน (authenticatio... |
| `quarkus-tdd` | none | ecosystem | ใช้ TDD กับ Quarkus 3.x LTS โดย JUnit 5, Mockito, REST Assured และ Camel testing พร้อมวัด coverag... |
| `ralphinho-rfc-pipeline` | none | ecosystem | รูปแบบการทำงานแบบ DAG ที่ขับเคลื่อนด้วย RFC สำหรับการประสานงาน multi-agent โดยมี quality gates แล... |
...
| `regex-vs-llm-structured-text` | none | ecosystem | กรอบการตัดสินใจสำหรับเลือกใช้ regex หรือ LLM ในการแยกวิเคราะห์ข้อความที่มีโครงสร้าง เริ่มต้นด้วย ... |
| `repo-scan` | none | ecosystem | สแกนซอร์สโค้ดทั้ง stack เพื่อตรวจสอบสินทรัพย์ทั้งหมด จำแนกประเภทไฟล์ ตรวจจับไลบรารีของบุคคลที่สาม... |
| `research-ops` | none | ecosystem | ทักษะนี้ช่วยให้คุณค้นหาข้อมูลล่าสุด เปรียบเทียบ หรือขอคำแนะนำจากแหล่งข้อมูลปัจจุบัน โดยใช้กระบวนก... |
...
| `rules-distill` | none | ecosystem | สแกนสกิลทั้งหมดเพื่อดึงหลักการที่ใช้ร่วมกันและกลั่นเป็นกฎ — สามารถเพิ่ม แก้ไข หรือสร้างไฟล์กฎใหม่ได้ |
| `rust-patterns` | none | ecosystem | รวบรวมแนวทางการเขียน Rust อย่าง Idiomatic ทั้ง Ownership, Error Handling, Traits, Concurrency และ... |
| `rust-testing` | none | ecosystem | กลยุทธ์ test Rust — unit, integration, async test, property-based test, doctest |
| `santa-method` | none | ecosystem | ใช้ผู้ตรวจสอบอิสระสองคน (review agents) ทำงานแบบ adversarial เพื่อตรวจสอบโค้ดหรือ output ก่อนส่งม... |
...
| `skill-comply` | none | ecosystem | ตรวจว่า skills/rules/agent definitions ถูก follow จริงไหม — สร้าง scenario test อัตโนมัติ |
...
| `skill-stocktake` | none | ecosystem | ใช้สำหรับตรวจสอบคุณภาพของสกิลและคำสั่งของ Claude รองรับโหมด Quick Scan (ตรวจเฉพาะสกิลที่เปลี่ยนแป... |
| `social-graph-ranker` | none | ecosystem | ให้คะแนนและจัดอันดับความสัมพันธ์ในโซเชียลกราฟ (X และ LinkedIn) เพื่อค้นหา warm intro, ประเมิน bri... |
...
| `springboot-patterns` | none | ecosystem | design pattern Spring Boot — REST API, layered services, data access, caching, async processing |
| `springboot-security` | none | ecosystem | รวบรวมแนวปฏิบัติที่ดีที่สุดสำหรับ Spring Security ครอบคลุมการยืนยันตัวตน (authentication), การกำห... |
| `springboot-tdd` | none | ecosystem | ใช้ TDD กับ Spring Boot โดยใช้ JUnit 5, Mockito, MockMvc, Testcontainers และ JaCoCo เพื่อเพิ่มฟีเ... |
| `swift-actor-persistence` | none | ecosystem | สกิลนี้ช่วยให้คุณจัดการข้อมูลแบบ thread-safe ใน Swift โดยใช้ actors เพื่อสร้าง in-memory cache ที... |
| `swift-concurrency-6-2` | none | ecosystem | แนวทาง Concurrency ใน Swift 6.2 ที่เข้าใจง่าย โดยค่าเริ่มต้นทำงานแบบ single-threaded ใช้ @concurr... |
...
| `unified-notifications-ops` | none | ecosystem | จัดการการแจ้งเตือนแบบรวมศูนย์จาก GitHub, Linear, desktop alerts และ hooks ต่าง ๆ ผ่าน workflow เด... |
...
| `workspace-surface-audit` | none | ecosystem | ตรวจ repo, MCP servers, plugins, connectors, env surfaces, harness setup แล้วแนะนำของที่ขาด/เกิน |
| `x-api` | none | ecosystem | สกิลนี้ช่วยให้คุณเชื่อมต่อกับ X/Twitter API เพื่อโพสต์ทวีต สร้างเธรด อ่านไทม์ไลน์ ค้นหา และดู ana... |
...
| `hexagonal-architecture` | none | ecosystem | ออกแบบ/ปรับ ports & adapters ให้ domain boundary ชัด — แยก core logic จาก external (DB, API, UI) |
| `implement` | build | mattpocock | ใช้ทักษะนี้เพื่อ implement ฟีเจอร์หรือแก้ไขโค้ดตาม PRD หรือ issues ที่กำหนด โดยเน้น TDD เพื่อให้โ... |
...
| `investor-materials` | none | ecosystem | สร้างและอัปเดต pitch deck, one-pager, investor memo, ใบสมัคร accelerator, financial model และเอกส... |
| `investor-outreach` | none | ecosystem | ช่วยร่างอีเมลเย็น (cold email), ข้อความแนะนำ (warm intro), อีเมลติดตามผล, อีเมลอัปเดตความคืบหน้า ... |
| `lead-intelligence` | none | ecosystem | ระบบวิเคราะห์และจัดลำดับความสำคัญของลีดแบบ AI-native ที่มาแทน Apollo, Clay และ ZoomInfo โดยใช้การ... |
...
| `agent-harness-construction` | none | ecosystem | ออกแบบ action space, tool definitions, observation formatting สำหรับ agent harness — ให้ agent ตั... |
| `agent-introspection-debugging` | none | ecosystem | debug agent failure แบบมีโครง — capture, diagnosis, contained recovery, prevention — ไม่ใช่แค่ลอง... |
...
| `cost-tracking` | none | ecosystem | ติดตามต้นทุนการใช้ model/token — รู้ว่างานไหนแพง และหาทางลด |
...
| Skill | Lifecycle | Category | Description |
|-------|-----------|----------|-------------|
| `energy-procurement` | none | ecosystem | ทักษะนี้ช่วยให้ AI Agent สามารถดำเนินการจัดหาพลังงานหมุนเวียน (Renewable Energy Procurement) สำหร... |
...
| Skill | Lifecycle | Category | Description |
|-------|-----------|----------|-------------|
| `carrier-relationship-management` | none | ecosystem | จัดการความสัมพันธ์กับผู้ให้บริการขนส่ง (carrier) อย่างเป็นระบบ ตั้งแต่การลงทะเบียน การตรวจสอบสถาน... |
| `customs-trade-compliance` | none | ecosystem | ทักษะนี้ให้ข้อมูลเกี่ยวกับพิธีการศุลกากรและการปฏิบัติตามข้อกำหนดทางการค้าระหว่างประเทศ เช่น HS Co... |
| `inventory-demand-planning` | none | ecosystem | ทักษะนี้ช่วยวางแผนความต้องการสินค้าคงคลัง โดยวิเคราะห์ข้อมูลยอดขายในอดีตและแนวโน้มตลาด เพื่อคาดกา... |
| `logistics-exception-management` | none | ecosystem | จัดการข้อยกเว้นในกระบวนการโลจิสติกส์ เช่น สินค้าชำรุด ส่งล่าช้า หรือที่อยู่ผิด ช่วยให้ทีมดำเนินกา... |
| `production-scheduling` | none | ecosystem | ช่วยวางแผนและจัดตารางการผลิตในระบบคลังสินค้าหรือโรงงาน โดยคำนึงถึงทรัพยากร เวลา และลำดับงาน เพื่อ... |
| `quality-nonconformance` | none | ecosystem | จัดการและติดตามรายการที่ไม่เป็นไปตามข้อกำหนดด้านคุณภาพ (Non-Conformance) ในกระบวนการโลจิสติกส์ ช่... |
| `returns-reverse-logistics` | none | ecosystem | จัดการกระบวนการคืนสินค้าและโลจิสติกส์ย้อนกลับ ตั้งแต่การสร้างคำขอคืนสินค้า การตรวจสอบสถานะ ไปจนถึ... |
...
|-------|-----------|----------|-------------|
| `cisco-ios-patterns` | none | ecosystem | รวบรวมรูปแบบคำสั่ง show, โครงสร้าง config, wildcard mask, การวาง ACL, การดูแล interface และการตรว... |
| `homelab-network-readiness` | none | ecosystem | เช็คลิสต์ความพร้อมของเครือข่าย homelab ก่อนปรับเปลี่ยน router, firewall, DHCP หรือ VPN ครอบคลุม V... |
| `homelab-network-setup` | none | ecosystem | คำแนะนำเชิงปฏิบัติสำหรับการวางแผนเครือข่ายภายในบ้านและ homelab ครอบคลุม gateway, switch, access p... |
| `homelab-pihole-dns` | none | ecosystem | จัดการ Pi-hole บน homelab ตั้งแต่ติดตั้ง ตั้งค่า blocklist, DNS-over-HTTPS, ผสาน DHCP, เพิ่ม loca... |
| `homelab-vlan-segmentation` | none | ecosystem | แบ่งเครือข่ายภายในบ้านเป็น VLAN สำหรับ IoT, แขก, อุปกรณ์ที่เชื่อถือได้ และเซิร์ฟเวอร์ โดยใช้ UniF... |
| `homelab-wireguard-vpn` | none | ecosystem | ตั้งค่า WireGuard VPN บน homelab เพื่อเชื่อมต่อเครือข่ายบ้านจากระยะไกลผ่านมือถือหรือแล็ปท็อป ครอบ... |
| `netmiko-ssh-automation` | none | ecosystem | ทักษะนี้ให้รูปแบบ Netmiko ที่ปลอดภัยสำหรับการเชื่อมต่อ SSH กับอุปกรณ์เครือข่าย รองรับการเก็บข้อมู... |
| `network-bgp-diagnostics` | none | ecosystem | รูปแบบการตรวจสอบ BGP แบบอ่านอย่างเดียว สำหรับวิเคราะห์สถานะ neighbor, การแลกเปลี่ยน route, นโยบาย... |
| `network-config-validation` | none | ecosystem | ตรวจสอบการตั้งค่าเครือข่ายก่อน deploy โดยตรวจจับคำสั่งอันตราย, ที่อยู่ซ้ำ, subnet ทับซ้อน, การอ้า... |
| `network-interface-health` | none | ecosystem | วินิจฉัยปัญหาของ network interface เช่น interface errors, drops, CRCs, duplex mismatches, flappin... |
...
| `nutrient-document-processing` | none | ecosystem | ประมวลผลเอกสารด้วย Nutrient DWS API รองรับการแปลงรูปแบบ OCR แก้ไข ลายเซ็น และกรอกข้อมูลในไฟล์ PDF... |
| `pptx` | none | uncategorized | ใช้เมื่อต้องทำงานกับไฟล์ .pptx ไม่ว่าจะเป็นการสร้างสไลด์เด็ค พิตช์เด็ค หรือพรีเซนเทชันใหม่ อ่านแล... |
| `visa-doc-translate` | none | ecosystem | แปลงเอกสารคำขอวีซ่าที่เป็นรูปภาพให้เป็นภาษาอังกฤษ และสร้างไฟล์ PDF สองภาษาที่มีทั้งต้นฉบับและคำแป... |
...
| Skill | Lifecycle | Category | Description |
|-------|-----------|----------|-------------|
| `canary-watch` | none | ecosystem | ใช้สำหรับตรวจสอบและยืนยัน URL ที่ deploy หลัง release — ตรวจสอบ HTTP endpoints, SSE streams, stat... |
| `ci-cd-and-automation` | ship | engineering-lifecycle | ตั้ง CI/CD pipeline — auto test, lint, build, deploy เพื่อลดมนุษย์ผิดพลาด |
| `deployment-patterns` | none | ecosystem | รวบรวมรูปแบบการ deploy, CI/CD pipeline patterns, Docker containerization, health checks, rollback... |
| `observability-and-instrumentation` | ship | engineering-lifecycle | เพิ่ม structured logging, RED metrics, OpenTelemetry tracing และ symptom-based alerting ระหว่างกา... |
| `opensource-pipeline` | none | ecosystem | ไพพไลน์สำหรับเตรียมโปรเจกต์ภายในให้พร้อมเผยแพร่เป็นโอเพนซอร์ส โดยทำงานผ่าน 3 เอเจนต์: forker (for... |
| `production-audit` | none | ecosystem | ตรวจสุขภาพระบบ production: logs, monitoring, alerting, runbook, SLO |

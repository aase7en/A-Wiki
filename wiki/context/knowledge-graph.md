# Knowledge Graph (auto-generated)

_Generated: 2026-06-08_  •  _Source: `.wiki-graph.json` (regen by `scripts/gen-index.py`)_

## Stats

- **Nodes**: 467
- **Edges**: 1225
- **Broken links**: 70 (run `python3 scripts/query-graph.py --broken`)
- **Orphans**: 170 (run `python3 scripts/query-graph.py --orphans`)

## Top Hubs (most-connected)

- `index-iot.md` (135 edges)
- `index.md` (100 edges)
- `wiki/entities/iot/esp32.md` (54 edges)
- `index-pharmacy.md` (37 edges)
- `wiki/entities/iot/esp32-s3.md` (35 edges)
- `index-ai-tools.md` (32 edges)
- `wiki/entities/iot/dx-lr02-lora.md` (31 edges)
- `index-env.md` (30 edges)
- `wiki/entities/iot/grafana.md` (27 edges)
- `wiki/synthesis/iot-lora-architecture.md` (27 edges)

## Query CLI

```bash
python3 scripts/query-graph.py --neighbors wiki/entities/iot/mqtt-protocol.md
python3 scripts/query-graph.py --orphans
python3 scripts/query-graph.py --hubs
python3 scripts/query-graph.py --broken
python3 scripts/query-graph.py --cross-domain
python3 scripts/query-graph.py --path FROM TO
```

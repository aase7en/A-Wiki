...
- date: recording date in YYYY-MM-DD (convert Thai Buddhist Era: subtract 543; if cell contains '"', '-', 'น', 'ง', copy from row above)
- time: time string as written (e.g. "15:00", "07:35น.")
- weight_kg: weight as float, null if blank. If cell is "5+5", return the SUM (10). Double check digits 2↔9, 6↔5, 1↔4, 8↔9, 4↔3, 2↔1 especially in the tens place. OPD weight is usually 10-39, rarely 40+.
...
- วอร์ด  = Ward (inpatient area) — WARNING: often misread as จอดรถ. Note: if recorder is อ้อย but weight <=10 in the evening, it might be Ward, cross-check!
- เวช    = เวชกรรม (medical ward) — WARNING: often misread as ลาว
- OPD   = ผู้ป่วยนอก (outpatient) — Recorder อ้อย is usually OPD.

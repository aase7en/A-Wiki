---
name: scrutinize
description: Outsider-perspective end-to-end review of a plan, PR, or code change. First questions intent and whether a simpler/more elegant approach would achieve the same goal, then traces the actual code path (not just the diff) to verify the change does what it claims. Enforces IRON LAW #1: NO PRODUCTION CODE WITHOUT A FAILING TEST FIRST.
---

# Scrutinize

> ⚠️ **IRON LAW #1 ENFORCED HERE**: NO PRODUCTION CODE WITHOUT A FAILING TEST FIRST.
> During review, verify that the change is accompanied by a failing test that validates the behavior.
> If code passes review but has no test, **REJECT until a failing test is written first.**
...
- **🚨 IRON LAW CHECK:** Is there a **failing test** that validates the intended behavior BEFORE the production code was written? If no → **REJECT.**
...
- **🚨 IRON LAW VIOLATION:** If production code exists without a corresponding failing test → blocker. Reject the change.

---
name: debug-mantra
description: Four-mantra debugging discipline — reproduce, trace the fail path, falsify the hypothesis, cross-reference every breadcrumb. Enforces IRON LAW #2: NO BUG FIXING WITHOUT ROOT CAUSE INVESTIGATION FIRST.
---

# Debug Mantra

> ⚠️ **IRON LAW #2 ENFORCED HERE**: NO BUG FIXING WITHOUT ROOT CAUSE INVESTIGATION FIRST.
> Superficial patching is FORBIDDEN. The four steps below are MANDATORY before any fix is proposed.
> If a parallel/secondary model attempts to patch without completing these steps, **DISCARD and REWRITE.**
...
## Iron Law Violence Trigger

If a **parallel agent** or **secondary model** proposes a fix without having completed the 4-step debug-mantra process:

1. **FLAG THE VIOLATION** — cite which step(s) were skipped.
2. **DISCARD THE OUTPUT** — do not merge, do not forward to user.
3. **REWRITE** — complete the debug-mantra process yourself before producing any fix.

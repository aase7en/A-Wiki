---
name: a-security
description: Evidence-led security guidance and vulnerability review adapted from Cloudflare/security-audit-skill. Use for focused security questions and explicit full/comprehensive codebase audits with trust-boundary coverage, machine-readable findings, independent verification, and bounded source/local-only validation.
domain: [security, engineering]
lifecycle_phase: review
category: pipeline
version: 1.0.0
license: MIT
origin: cloudflare/security-audit-skill@c1c8a8c1471069fb0e188eeaff69b8e8db6564a8
invocation: manual
status: canonical
---
...
## A-Wiki integration contract

This skill is an A-Wiki adaptation of Cloudflare's `security-audit-skill`. Upstream is tracked by `scripts/refresh-cloudflare-security-audit.py` and snapshotted under `skills/_upstream/cloudflare-security-audit/`. The adopted baseline is pinned in `origin` above; the Cloudflare MIT notice is preserved in `LICENSE.cloudflare`.

A-Wiki authority remains higher than this imported workflow:

- recover current repo/Work Order/claim/runtime truth before any mutation; this skill never grants mutation authority;
- keep guidance mode as the default and enter the six-phase full audit only when the user explicitly requests a full/comprehensive audit, pen test, or report artifacts;
- never weaken A-Wiki privacy, secret, PHI, ownership, branch, review, or exact-SHA gates;
- for A-Wiki-managed repositories, keep audit outputs outside tracked public Git by default and never silently promote findings into `wiki/` or memory;
- use A-Wiki's proven Worker/model routing for delegated roles, while preserving finder/verifier independence;
- target-controlled execution remains fail-closed unless every sandbox control required below is actually enforceable;
- do not probe live/shared targets, production identities, external control planes, or paid/shared services;
- on native Windows, do not weaken the upstream validators' fail-closed `O_NOFOLLOW` / `O_NONBLOCK` requirement. If those primitives are unavailable, run final file-based validation in a supported Linux/POSIX runner with Node or record validation as blocked; guidance/source review may still proceed.

When upstream and A-Wiki rules differ, preserve the stricter safety/privacy/authority rule and record the delta during upstream refresh.

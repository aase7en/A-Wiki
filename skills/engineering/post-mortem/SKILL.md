---
name: post-mortem
description: Write the canonical engineering record of a fixed bug — root cause, mechanism, fix, validation, and how it slipped through. Engineer-audience, code identifiers welcome. Use after a debug session lands a fix, before closing the ticket.
---
...
- **Code identifiers are first-class.** `tadaLaunchPrepare`, `scratchBuf` — keep them. The whole point is that future engineers can grep their way back to the change.
...
- **No advocacy.** A post-mortem records what happened and what's next.

## Output flow

1. **Confirm all four required inputs are satisfied.** If any are missing, list them and stop. Do not draft.
2. **Confirm where it goes** (default: JIRA comment). Other valid destinations: PR description, `docs/postmortems/<ticket>.md`, internal wiki page.
3. **Produce the draft** as a single chat block.
4. **Sign-off before posting.** If posting back to JIRA, wait for explicit *"post it"* / *"go ahead"* / *"yes,"* then post. Print-only output needs no approval.

...
- **🧩 Skills tab** — universal second-brain view: **360/365 skills (98%)** with **Thai descriptions**, **when-to-use triggers**, **1-2 examples**, and **process-step simulation**. Filter by agent to see exactly what *your* AI agent can do. Auto/manual invocation badges, **installed-vs-catalog** badges (✅ on disk / 📦 catalog-only), and **📋 copy-invocation** buttons make it instant to go from "see a skill" to "use it". **🔗 Related skills** in each detail drawer chain exploration by lifecycle phase + domain.
- **🎬 Process simulation + multi-skill flows** — click any skill with `process_steps` → animated walkthrough of each step as a glowing station. The **🎬 Multi-skill flows** bar (top of Skills tab) shows **8 end-to-end workflows** (deploy Flask, debug prod, secure API, migrate DB, refactor module, ship feature, ingest paper, audit agent architecture) — each chains 6 real skills in order with Thai labels, so non-technical users see *how A-Wiki orchestrates* a real job.
- **👁 `?agent=` auto-detection** — `bash scripts/dashboard-ensure.sh zcode` (or let SessionStart detect from env) opens the dashboard with `?agent=zcode` so the Skills tab auto-filters to *only skills ZCode can invoke* — instant answer to "what does MY agent know?"

...
- **🧩 Skills tab** — universal second-brain view: **367/367 skills (100%)** with **Thai descriptions**, **when-to-use triggers**, **1-2 examples**, **process-step simulation**, and **📋 copy-invocation** buttons backed by registry `invocation_hint` (359/367 = 98% filled). Auto/manual invocation badges, **installed-vs-catalog** badges (✅ on disk / 📦 catalog-only), and **🔗 Related skills** (by lifecycle phase + domain) in each detail drawer.
- **🎬 Process simulation + multi-skill flows** — click any skill with `process_steps` → animated walkthrough of each step as a glowing station. The **🎬 Multi-skill flows** bar (top of Skills tab) shows **16 end-to-end workflows** (deploy Flask, debug prod, secure API, migrate DB, refactor module, ship feature, ingest paper, audit agent architecture, **onboard new dev/machine, write new skill, review PR, upgrade dependency, investigate perf regression, respond to security advisory, cleanup stale skills**) — each chains 4-6 real skills in order with Thai labels, so non-technical users see *how A-Wiki orchestrates* a real job.
- **📊 Coverage tab** — registry health at a glance: bar chart of coverage % across 6 v2 fields (`th_description`, `when_to_use`, `examples`, `process_steps`, `invocation_hint`, `invocation`), broken down by domain (21), agent (12), and lifecycle phase (8). Color-coded heatmaps (green ≥90%, yellow 60-89%, red <60%) + clickable "still missing" lists that jump to the Skills tab for quick-fill. Self-reinforcing loop: the gaps you see drive the next batch-fill run.
- **👁 `?agent=` auto-detection** — VS Code's `folderOpen` task calls `bash scripts/dashboard-ensure.sh "$(bash scripts/detect-agent.sh)"`, so opening the workspace auto-launches the dashboard filtered to *only skills your current agent can invoke*. The same `agent_detect.py` module is used by the SessionStart hook and any shell caller — one source of truth across Claude/Codex/ZCode/Gemini/Cursor/Windsurf/Cline/Antigravity/Hermes/Kilo/Copilot (11 agents).
- **🧬 Skills Expansion v4 (9 features)** — the Skills tab is now a full skill-exploration environment:
  - **🧬 Skill dependency graph** — toggle List↔Graph; the graph uses the already-loaded vis-network to draw skill relationships by lifecycle phase (same phase → weight 3) and shared domains (2+ → weight 2). Default shows the ~40 skills with a real lifecycle phase; `?all=1` shows all 387. Click any node → detail drawer.
  - **📥 Sim export** — the process-simulation modal now has **SVG** and **PNG** download buttons. Output is a reproducible horizontal pipeline diagram generated from step *data* (not DOM capture), so it's clean, re-renderable, and theme-independent. Shareable in docs/presentations.
  - **⌨️ Keyboard shortcuts** — `/` focuses Skills search · `g s/c/g/f/t/h` switch tabs (Skills/Coverage/Graph/Flow/Timeline/Home) · `Esc` closes the topmost overlay · `?` opens the shortcuts help modal. Power-user navigation without the mouse.
  - **⚖️ Skill comparison** — checkbox on each card adds it to a floating compare bar (max 3); click **เปรียบเทียบ** to open a side-by-side table of th_description / when_to_use / invocation_hint / domain / phase / process_steps count / version / installed status. Last comparison saved to localStorage.
  - **📊 Walkthrough difficulty** — each of the 16 multi-skill flows now carries an auto-scored **0-100 difficulty** (step count + skill complexity from security/review/ship phases + process_steps richness + phase diversity), shown as a colored badge (เริ่มต้น/ปานกลาง/ขั้นสูง). If the auto-scored level differs from the manual `level_th`, both are shown.
  - **📈 Trending (7-day)** — the discovery bar now shows your top-5 most-opened skills in the last 7 days (from `awiki-skill-opens` localStorage log, max 200 entries, FIFO). Pure client-side — no server tracking, no data leaves the browser.
...
- **🎛️ Skills Expansion v6 (9 features + 1 bugfix)** — power-user UX + real-usage intelligence:
  - **🐛 Walkthrough recommender bugfix** — the v5 "recommender → flow suggestions" feature was silently broken (the walkthrough loader had no `def` header, so `NameError` was swallowed by try/except and flows were always empty). Now fixed + pinned with a regression test.
  - **🔍 Command Palette (Ctrl/⌘+K)** — press Ctrl+K anywhere (even while typing) to open a fuzzy-search palette over all skills, 9 views, and keyboard shortcuts. Subsequence scoring (no dependency), capped at 10 results, 150ms debounce, arrow-key nav + Enter to activate.
  - **🩺 Skill health score** — each skill now has a 0-100 health score from weighted coverage of 6 documentation fields (`th_description`=3, `when_to_use`=2, `examples`=2, `process_steps`=2, `invocation_hint`=1, `invocation`=1). Colored badge per card (🔴 วิกฤต / 🟠 อ่อน / 🟢 พอใช้ / 💚 ดี) + sort dropdown (weakest-first surfaces under-documented skills).
  - **📡 Registry push (SSE auto-refresh)** — a background mtime watchdog detects changes to `skills-registry.json` and broadcasts an SSE `registry_update` event; connected dashboards silently reload the active view (Skills/Coverage) without a manual refresh. Payload carries only type+ts (no skill data over SSE).
  - **🔄 Cycle detection** — the skill graph view now checks for circular dependencies via bounded DFS (max_depth=10) and shows a ⚠️ banner listing any cycles found (`A → B → A`). Current real graph: 0 cycles.
  - **⭐ Filter presets** — save the current filter combination (agent + domain + invocation + category + installed + sort) as a named preset, recalled with one click from a dropdown. localStorage `awiki-filter-presets` (max 10, dedupe by name).
  - **📥 Multi-agent matrix CSV export** — download a skills × agents visibility matrix as CSV (402 skills × 11 agents, 1/0 per cell). Uses `Registry.canonical_for_agent()` per agent — spots per-agent gaps (e.g. zcode has 28 skills no other agent sees).
  - **📊 Co-occurrence mining** — toggle the skill graph between "phase/domain adjacency" (default) and "used together in reality" (mined from `awiki-skill-opens` log — pairs opened within 60s, weight≥2 threshold). Node size scales with open count, edge width with co-occur count.
  - **🌗 Auto theme (prefers-color-scheme)** — the theme toggle is now 3-state (auto → dark → green-white → auto). `auto` follows the OS `prefers-color-scheme` live (no reload on OS theme change). One-time migration from legacy 2-state `awiki-theme`.
  - **🕘 Search history** — the Skills search box remembers recent queries (localStorage `awiki-search-history`, max 20, 30-day TTL, dedupe) and shows them as clickable chips when the input is empty and focused. 🗑 clears all.

- **🎛️ Skills Expansion v7 (9 features)** — power, intelligence, and collaboration layer:
  - **🔗 URL state sync** — every filter, tab, and sort now reflects into the URL (`?view=skills&agent=zcode&sort=health-asc&q=tdd`). Copy the URL via the "🔗 Share URL" button and reopen on another machine — the exact dashboard state restores. Browser back/forward works (popstate handler). Idempotent sync (replaceState, no history spam).
  - **⌨️ Shortcut customization** — remap any keyboard shortcut via the "⚙️ ปรับแต่ง" modal in the shortcuts help (?). Capture a new key sequence (single or `g x` two-key), export/import your profile as JSON, reset to defaults. localStorage `awiki-keybindings`. The main keydown handler resolves user overrides before defaults.
  - **🔔 Notification system** — opt-in desktop notifications (Web Notification API) for: registry updates, hook blocks, cycle detection, low-health skills (≥3 critical), and delegation failures. Per-event toggles in the 🔔 modal; falls back to toast when permission denied. localStorage `awiki-notif-prefs`.
  - **📅 Skill lifecycle (first_seen)** — `skill_history()` now also returns `first_seen` (the git commit date that first added the SKILL.md, via `git log --diff-filter=A`). Shown in the detail drawer ("📅 เพิ่มเมื่อ YYYY-MM-DD") with a 🆕 "ใหม่" badge if added within 30 days. Lightweight — no new persistence, git is the source of truth.
  - **🤖 Smart default agent** — new `/api/detect-agent` endpoint wraps the existing `scripts/lib/agent_detect.py:detect_agent()`. On boot, if the URL has no explicit `?agent=`, the dashboard auto-detects the calling agent from env fingerprints and pre-filters the Skills tab, showing a "🤖 auto-detected: zcode (จาก env)" banner with a clear-to-all link. `AWIKI_DASHBOARD_AGENT` env override wins.
  - **💾 Workspace save/restore** — snapshot the current layout (active view + scroll position + all Skills filters) into a named workspace (localStorage `awiki-workspaces`, max 10). Restore on demand or auto-restore the last workspace on boot (opt-in checkbox). The 💾 header button opens the workspace menu.
  - **📋 Skill review queue** — the Coverage tab now has a "Review Queue" card listing low-health skills (score < threshold, default <60) sorted worst-first, with a ✅ done-checkbox (localStorage `awiki-review-done`), a progress bar (done/total), and a ✏️ inline-edit button that jumps straight to fixing the missing field. Powered by new `review_queue()` in skills_service.

...
def render_readme(payload: dict[str, Any], module_name: str, scene_name: str, scene_key: str) -> str:
    summary = payload.get("summary") or {}
    return "\n".join(
        [
            "# Generated Phaser Asset Pack",
            "",
            "Generated by `scripts/game/bootstrap_phaser_asset_pack.py`.",
            "",
            "## Files",
            "",
            f"- `{module_name}.json` — payload from asset manifests",
            f"- `{module_name}.ts` — `preloadPhaserAssets(scene)` and `registerPhaserAnimations(scene)`",
            f"- `{scene_name}.ts` — preview scene stub using scene key `{scene_key}`",
            "- `index.ts` — barrel exports when copied into a project folder",
            "",
            "## Summary",
            "",
            ...
            "## Usage",
            "",
            f"Import `{scene_name}` into your Phaser game config or route it through your existing scene registry.",
            "",
        ]
    )
...
def bootstrap_pack(
    ...
    readme_text = render_readme(payload, module_name=module_name, scene_name=scene_name, scene_key=scene_key)
    ...
    if copy_to_project is not None:
        ...
        project_readme_path = copy_to_project / readme_path.name
        ...
        shutil.copy2(readme_path, project_readme_path)
        ...
        result.update(
            {
                ...
                "project_readme_path": project_readme_path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[OK] readme  -> {result['readme_path']}")
    if "project_loader_path" in result:
        ...
        print(f"[OK] project readme  -> {result['project_readme_path']}")

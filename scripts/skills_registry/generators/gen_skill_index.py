...
def render(registry: Registry) -> str:
    ...
    out.append(">")
    out.append("> **Search-first (Slice 2):** run MCP `skill_route` (trigger + description")
    out.append("> fallback) or the A-ROUTER trigger table BEFORE scanning this file.")
    out.append("> This index is a REFERENCE for choosing manually — read only the domain")
    out.append("> section you need, not the whole table at session start.")
    out.append("")

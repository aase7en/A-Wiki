...
Usage:
  ...
  python3 scripts/compare_delivery.py \\
    --delivery drive/pharmacy/deliveries/20260520_SCH_ORNC2605P010859.json \\
    --export excel

Input (stdin or file): รายการที่สั่ง 1 บรรทัด/รายการ (รองรับ "name: qty" หรือ "name qty")

Delivery invoices + comparison exports are real pharmacy business data and
live only in drive/pharmacy/ (not in the tracked wiki/ tree) — resolved via
scripts/drive_path.py::get_pharmacy_dir().
"""
...
def export_excel(result: dict, delivery_meta: dict, merges: list[dict]) -> pathlib.Path:
    ...
    try:
        exports_dir = get_pharmacy_dir() / "exports"
    except DriveNotLinkedError as e:
        print(f"⚠️  Cannot export Excel: {e}", file=sys.stderr)
        sys.exit(1)
    exports_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = exports_dir / f"compare_{ts}.xlsx"

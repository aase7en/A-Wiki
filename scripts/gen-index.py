...
def _extract_graph_hygiene() -> dict | None:
    """Read live graph counters for wiki/context/graph-hygiene.md.

    Preferred source: the cap-builder JSON (already classified by domain,
    reuses read_graph_hygiene() logic). Fallback: read .wiki-graph.json
    directly with minimal classification, so this works even when the
    cap-builder is absent or fails. Returns None when no graph data exists.
    """
    ...
    if cap_builder.exists():
        ...
        try:
            ...
            print(f"[WARN] graph-hygiene extraction (cap-builder) failed: {e}", file=sys.stderr)
    ...
    try:
        ...
        print("[WARN] graph-hygiene extraction: .wiki-graph.json corrupt", file=sys.stderr)
        return {"status": "invalid-json", "nodes": 0, "edges": 0, "broken_links": 0, "orphans": 0,
                "broken_by_domain": {}, "orphan_by_domain": {}, "orphan_samples": []}
...
def render_graph_hygiene(gh: dict) -> str:
    """Render the Knowledge Graph Hygiene section as a standalone markdown file.

    This was extracted OUT of wiki-capability-map.md so that the capability
    map (a stable catalog) is not coupled to volatile live graph counters,
    which made gen-index.py --check non-deterministic. This file is generated
    alongside the capability map but is excluded from the --check gate
    (see main(): it is written directly, not added to the outputs dict).

    See ADR on separating stable catalog from volatile live metrics.
    """
    status = gh.get("status", "ok")
    status_line = {
        "ok": "_Status: live, rebuilt each `gen-index` run._",
        "missing": "_Status: `.wiki-graph.json` not found — run `python scripts/gen-index.py` to build it._",
        "invalid-json": "_Status: `.wiki-graph.json` is corrupt — rebuild with `python scripts/gen-index.py`._",
    }.get(status, f"_Status: {status}_")
    ...
    lines = [
        ...
        "| Metric | Count |",
        "|---|---:|",
        f"| Nodes | {gh.get('nodes', 0)} |",
        f"| Edges | {gh.get('edges', 0)} |",
        f"| Broken links | {gh.get('broken_links', 0)} |",
        f"| Orphans | {gh.get('orphans', 0)} |",
        "",
    ...
    broken = gh.get("broken_by_domain") or {}
    if broken:
        for domain, count in broken.items():
            lines.append(f"| {_md_escape(domain)} | {count} |")
    else:
        lines.append("| none | 0 |")
    lines.extend(["", "## Orphans by domain", "", "| Domain | Count |", "|---|---:|"])
    orphans = gh.get("orphan_by_domain") or {}
    if orphans:
        for domain, count in orphans.items():
            lines.append(f"| {_md_escape(domain)} | {count} |")
    else:
        lines.append("| none | 0 |")
    samples = gh.get("orphan_samples") or []
    if samples:
        lines.extend(
            [
                "",
                "## Orphan samples",
                "",
                ", ".join(f"`{_md_escape(item)}`" for item in samples[:10]),
            ]
        )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    # Extract live graph counters into a SEPARATE file wiki/context/graph-hygiene.md,
    # excluded from the --check gate. Those counters shift every gen-index run,
    # making the capability map non-deterministic if embedded. Preferred source is
    # the cap-builder JSON (already classified by domain); fallback reads
    # .wiki-graph.json directly so this works even if the cap-builder is absent.
    # See render_graph_hygiene() and the ADR.
    graph_hygiene_data = _extract_graph_hygiene()
    ...
    # Write the volatile graph-hygiene file SEPARATELY (not in outputs dict, so
    # it is excluded from the --check gate). Its content changes every run by
    # design (live counters); see render_graph_hygiene() and the ADR.
    if graph_hygiene_data is not None:
        hygiene_path = CONTEXT_DIR / "graph-hygiene.md"
        hygiene_path.write_text(render_graph_hygiene(graph_hygiene_data), encoding="utf-8")
        print(f"[OK] Wrote {hygiene_path.relative_to(REPO_ROOT)} (excluded from --check)")

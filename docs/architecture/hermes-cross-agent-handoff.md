...
- Host: `umbrel-1.tail<id>.ts.net` (Tailscale, `100.x.y.z`), ports 22/443/80 open.
...
### CHUNK E (hermes-e) — Phase 3 RESULTS ✅ DONE (2026-07-07)

Deployed live on the Pi5 via `drive/private-tools/hermes-e/deploy-pi5.py` (after switching to a Tailscale-friendly network). All 7 command-skills are now wired on-device and the gateway has rescanned them.

| Metric | Target | Result |
|--------|--------|--------|
| Canonical clone HEAD | current main (`1100098`) | **`1100098`** ✅ (was `a37e491` — FF'd via fetch + stash/pop) |
| 7 command-skill symlinks | all → `/opt/data/A-Wiki/skills/awiki/<name>` | **7/7** ✅ (wiki, search, review, spec, plan, build, ship) |
| Broken symlinks under `/opt/data/skills` | 0 | **0** ✅ |
| Router + command map on device | present | ✅ `telegram-command-router.{py,sh}` + `telegram-commands.json` (12506 + 715 + 1261 bytes) |
| Gateway restarted | s6 auto-restart after HUP | ✅ PID **151 → 2002**, telegram=connected |
| End-to-end router test on device | `/wiki mqtt broker` → JSON hits | ✅ 5 FTS5 hits returned (mqtt-protocol-overview, emqx, mosquitto, publish-subscribe, esp32-hx711-mqtt) |

#### Sub-step outcomes

- **Snapshot** ✅ — `/opt/data/.hermes-e-skills-snap-20260707-113125.tar.gz` (rollback anchor, hermes-owned).
- **FF canonical clone** ✅ — HEAD was already at `1100098` (chunk E commits pushed before deploy). The `git stash --include-untracked` + `ff-only` + `stash pop` path hit the known C3' failure mode: 17 auto-gen files (`wiki/context/*`, `CLAUDE.md` indexes, `.wiki-graph.json`, `brain-map.canvas`) conflicted on `generated_at` timestamps. **Resolved by `git checkout HEAD -- <file>` + `git add`** for each `UU` file (HEAD version is correct + machine-regenerated by `gen-index.py` anyway — same call as C3'). Untracked device-only files (`scripts/investment/`, glmic summaries) preserved.
- **7 symlinks** ✅ — `ln -sfn` created all 7 in `/opt/data/skills/awiki/`. (First deploy-script run hit a bash syntax error: missing `;` before `then` in the inline for-loop. Fixed in `drive/private-tools/hermes-e/deploy-pi5.py` for future runs; the manual run with the corrected command succeeded.)
- **Verify** ✅ — 0 broken symlinks, all 7 resolve to readable SKILL.md, HEAD at `1100098`.
- **Gateway rescan** ✅ — sent `kill -HUP` to gateway PID 151 (read from `/opt/data/gateway.pid` JSON). s6 supervisor (`s6-supervise gateway-tech_and_ai_architect`) auto-restarted the gateway as PID 2002, telegram platform reconnected. The 7 new skill dirs are now scanned into the gateway's skill registry.

#### On-device router sanity (the real proof)

Ran the router INSIDE the container to confirm the full path works end-to-end:
```
$ cd /opt/data/A-Wiki && python3 scripts/hermes/telegram-command-router.py \
    --message "/wiki mqtt broker" --apply
[
  {"path": "wiki/sources/iot/mqtt-protocol-overview.md", "title": "MQTT Protocol Overview", ...},
  {"path": "wiki/entities/iot/emqx.md", ...},
  ...5 hits...
]
```

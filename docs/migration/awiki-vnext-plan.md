...
- **Targeted sweep**: 278 passed across all remediation areas. **Canonical full suite after remediation: 0 failed / 2,595 passed / 17 skipped** (459.36s; +11 vs pre-remediation = remediation tests). **GitHub PR CI evidence (R-P2-001)**: PR [#11](https://github.com/aase7en/A-Wiki/pull/11) opened `refactor/awiki-kernel-vnext-clean` → `main`; the initial `pull_request` webhook was lost to a GitHub API incident (repeated 503s — PR creation itself only succeeded via REST), so this documentation commit re-fires `pull_request: synchronize` to trigger `ci-core` from the merge ref. Run status is recorded in the re-review handoff.

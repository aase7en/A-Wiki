...
11. **Declare work before touching a shared surface** — every agent (Claude, ZCode, Codex, Gemini, Cline, any future one) must register scope + goal + phase via MCP `claim_acquire` before editing `skills/`, `scripts/`, `commands/`, `agents/`, `skills-registry.json`, `AGENTS.md` or harness configs, and must call `claim_list` before starting anything non-trivial. A PreToolUse hook **blocks** (exit 2) an edit landing inside another agent's live claim and names the holder, their goal and their phase. Claims carry a TTL lease and self-reap, so a crashed agent cannot lock the repo; the Stop hook releases them. Enforced by `scripts/hooks/check_agent_claim.py`; released by `scripts/hooks/release_agent_claims.py`. Override (emergencies only, prints BYPASSED): `AWIKI_CLAIM_GATE=0` or `HOOK_SKIP=check_agent_claim`. **Rationale:** 2026-07-27 — Claude and ZCode independently built the same intent router and the same phase state machine, in this repo, on `main`, during the same hours; neither knew until both had landed. Every primitive needed to prevent it already existed (TaskBoard leases, Blackboard, shared `.tmp/`) — what was missing was a gate. See `skills/awiki/a-claim/SKILL.md`. **Corollary — commit early:** the same day, a mid-session `git pull --rebase` (SessionStart hook) silently discarded uncommitted edits to five tracked files. With auto-commit and auto-pull hooks running, uncommitted work is not safe; commit each coherent batch immediately. Cross-machine coordination is NOT yet covered (`.tmp/` is local) — use the blackboard and commit messages there.
...
|-------|----------|------------|-------|
| **1. Enforcement & Discipline** | What agents *must/must not* do — hard-enforced | **9 Iron Laws** (above) · **18 PreToolUse hooks** (`scripts/hooks/`: `check_skill_registry`, `check_source_original_file`, `check_test_before_code` [Iron Law #1 hard-enforce], `check_harness_routing`, `check_output_format`, `check_raw_immutable`, `check_external_editor_drift`, `check_secret_leak`, `check_bash_no_branch` …) · **3 Stop advisory hooks** (`self_audit`, `a_loop_distill` [a-loop Phase 3], `check_verify_before_done` [verify-before-done gate]) · **9arm skills** (`debug-mantra` is the named instrument of Iron Law #2; + `scrutinize`, `post-mortem`) · eval/safety personas (`agents/test-engineer.md`, `agents/security-auditor.md`) | 🔴 HARD + 🟡 SOFT |

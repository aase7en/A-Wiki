...
11. **Declare work before touching a shared surface** — every agent (Claude, ZCode, Codex, Gemini, Cline, any future one) must bind an exact task id and scope before editing shared surfaces. **COLLAB.md + Git is the canonical durable cross-machine ownership authority**; MCP `claim_acquire` requires `task_id` and writes/validates durable ownership first, then mirrors a same-machine TTL cache. `claim_list`, `claim_advance`, and `claim_release` operate on that derived cache only; cache expiry/release never releases durable ownership. `check_agent_claim.py` blocks foreign durable claims and only `RECONCILED` foreign cache rows; `PARTIAL_UNRECONCILED`/legacy TTL-only rows are never ownership authority. Before non-trivial work recover `COLLAB.md`/WO/branch/PR, then use the exact existing task id; when the chunk ends, update/release the same durable COLLAB row through Git. Override (emergencies only, prints BYPASSED): `AWIKI_CLAIM_GATE=0` or `HOOK_SKIP=check_agent_claim`. **Rationale:** 2026-07-27 duplicate parallel implementation proved a same-machine-only lease is insufficient; durable Git-backed identity now closes the cross-machine gap while TTL remains an acceleration cache. See `skills/awiki/a-claim/SKILL.md` and `docs/protocols/cross-agent-work-orders.md`.
